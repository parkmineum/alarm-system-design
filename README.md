...
요구사항 자체가 아니라 *"왜 이걸 골랐는가"* 를 적는다. 비교한 대안과 탈락 사유를 함께 남겨, 같은 자리에서 다시 골라도 같은 결론에 도달하게 만든다.
...
DB 가 이미 영속성과 분산 락(`SELECT ... FOR UPDATE`)을 동시에 제공하므로 outbox 패턴이 **가장 적은 추가 인프라로 두 요구사항을 모두 만족** 한다. 거기에 dispatcher 추상을 두면 미래에 Kafka 등으로 갈아탈 때 빈 교체 한 곳만 바꾸면 된다 — 점진적 마이그레이션 경로 자체가 outbox 로 깔린다.

전체 비교는 [docs/async-and-retry.md §3](docs/async-and-retry.md) 참고.

---

### 2. 멱등성 키 — 클라이언트 vs 서버 생성

호출자가 키를 직접 보내는 모델도 가능했다. 두 옵션의 위험은 다르다.
...
서버에서 결정적으로 생성하면 호출자는 *"같은 이벤트 = 같은 인자"* 만 보장하면 된다. 키 누락이라는 실패 모드 자체가 사라진다.

```
idempotencyKey = sha256(eventType + ':' + refType + ':' + refId + ':' + recipientId + ':' + channel + ':' + scheduledAt)
```

`scheduledAt` 도 키에 포함했다 — 같은 이벤트라도 다른 시각에 예약 발송하는 케이스 (D-1 알림 / D-3 알림) 가 별개 row 로 다뤄져야 하기 때문.
...
ShedLock 같은 *스케줄러 단위* 락은 결국 동시성을 1로 떨어뜨린다 — 워커를 N 개 띄워도 한 번에 한 인스턴스만 일한다. 처리량이 워커 수에 의존하지 않아 수평 확장의 의미가 옅어진다.

`FOR UPDATE SKIP LOCKED` 는 **row 단위** 락이다. 워커마다 락이 걸리지 않은 다음 row 를 가져가므로 처리량이 워커 수만큼 선형으로 증가한다. 락 보유 시간을 짧게 (`PROCESSING` 마킹만 하고 즉시 commit, 실제 외부 호출은 락 밖) 유지하면 락 경합도 거의 없다.
...
MySQL 8.0.1+ 제약은 `docker-compose.yml` 의 `mysql:8.0` 이미지로 충족.
...
수동 재시도 시 `autoAttemptCount` 를 어떻게 다룰지가 핵심 트레이드오프였다.
...
세 시점이 가능했다. 트레이드오프는 *"최신 템플릿 사용"* 과 *"실패 발견 시점"* 사이에 있다.
...
`rendered_body` 와 `template_version` 이 `notification` row 에 함께 저장되므로 — 추후 템플릿이 수정되어도 *"실제로 무엇을 보냈는지"* 가 추적 가능하다.

`notification` 과 `notification_template` 을 FK 로 묶지 않은 이유도 같다. FK 로 묶고 템플릿을 수정하면 과거 발송 row 의 의미가 흔들린다. 스냅샷 분리가 더 안전하다.

---

### 6. 다중 디바이스 읽음 처리 — 조건부 UPDATE

같은 알림에 다른 디바이스가 동시에 `PATCH ../read` 를 보내면 `read_at` 이 마지막 요청 시각으로 덮어 씌워질 수 있다. SELECT 후 UPDATE 패턴은 락이 필요하고, 무조건 UPDATE 는 첫 읽음 시각이 손실된다. 조건부 UPDATE 한 줄이 셋을 동시에 해결한다.
...
`affected rows = 1` 이면 최초 읽음, `0` 이면 이미 읽음 (둘 다 `200 OK`, no-op). DB row write lock 이 자동 직렬화하므로 별도 락 불필요.
...
요구사항이 *"실제 운영 환경 전환 가능한 구조"* 를 요구했다. 미래에 Kafka 등으로 갈아탈 수 있도록 swap 지점을 미리 노출한다.
...
상세는 [docs/async-and-retry.md §9](docs/async-and-retry.md).

---

### 8. 처리 타임아웃 임계값 — 왜 5 분인가

워커가 row 를 `PROCESSING` 으로 바꾼 직후 비정상 종료되면, 그 row 는 영원히 `PROCESSING` 상태로 남는다. 복구 잡(`@Scheduled`) 이 임계값을 넘긴 row 를 `PENDING` 으로 되돌린다.
...
복구된 row 는 `autoAttemptCount` 를 증가시키지 않는다. 외부 호출 결과를 알 수 없는 상태이므로, 카운터를 올리면 정상 동작 가능한 row 가 빠르게 DEAD_LETTER 로 가는 부작용이 생긴다.
...
예약 발송을 별도 스케줄러 인프라(예: Quartz)로 둘지, 컬럼 한 개로 통합할지 선택지가 있었다.

`scheduled_at` 컬럼 하나로 통합하면 — 즉시 발송과 예약 발송이 **같은 코드 경로** 를 탄다.
...
별도 스케줄러를 두면 두 시스템(스케줄러 + outbox)의 정합성을 신경 써야 한다 — 컬럼 통합은 복잡도를 한 단계 낮춘다.

---

### 10. 타인 알림 접근 — 404 vs 403

다른 사용자의 알림 ID 로 GET 했을 때, "권한 없음(403)" 으로 응답하면 ID 가 존재한다는 정보가 노출된다. 공격자가 ID 를 enumerate 해 *"누가 어떤 알림을 받았는지"* 를 추론할 수 있다.

`404 NOTIFICATION_NOT_FOUND` 로 통일했다. 존재 여부 자체가 외부에 노출되지 않는다.
...
같은 정책을 admin API 가 아닌 모든 사용자 API 에 일관 적용한다.

---

## 검증 결과

`./gradlew test` 한 번으로 전체 검증이 돌아간다. 총 **93 건** 통과, 0 실패. 슬라이스를 의도적으로 분리해 어떤 레이어에서 실패가 났는지 즉시 식별 가능하다.
