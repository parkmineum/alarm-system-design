...
요구사항 자체가 아니라 *"왜 이걸 골랐는가"* 를 정리했습니다. 비교한 대안과 탈락 사유를 함께 남겨두면, 같은 자리에서 다시 고민하더라도 같은 결론에 도달할 수 있다고 봤기 때문입니다.
...
서버에서 결정적으로 생성하면 호출자는 *"같은 이벤트 = 같은 인자"* 만 보장하면 되고, 키 누락이라는 실패 모드 자체가 사라진다고 봤습니다.
...
ShedLock 같은 *스케줄러 단위* 락은 결국 동시성을 1로 떨어뜨리는 구조라고 봤습니다 — 워커를 N 개 띄워도 한 번에 한 인스턴스만 일하게 됩니다. 처리량이 워커 수에 의존하지 않으면 수평 확장의 의미가 옅어집니다.
...
수동 재시도 시 `autoAttemptCount` 를 어떻게 다룰지가 가장 핵심적인 트레이드오프라고 봤습니다.
...
세 시점을 후보로 두고 비교했습니다. 트레이드오프는 *"최신 템플릿 사용"* 과 *"실패 발견 시점"* 사이에 있다고 봤습니다.
...
같은 알림에 다른 디바이스가 동시에 `PATCH ../read` 를 보내면 `read_at` 이 마지막 요청 시각으로 덮어 씌워질 수 있습니다. SELECT 후 UPDATE 패턴은 락이 필요하고, 무조건 UPDATE 는 첫 읽음 시각을 잃게 됩니다. 조건부 UPDATE 한 줄로 세 가지를 동시에 해결할 수 있다고 봤습니다.
...
복구된 row 는 `autoAttemptCount` 를 증가시키지 않도록 했습니다. 외부 호출 결과를 알 수 없는 상태이기 때문에, 카운터를 올려버리면 정상 동작 가능한 row 가 빠르게 DEAD_LETTER 로 떨어지는 부작용이 생긴다고 봤습니다.
...
다른 사용자의 알림 ID 로 GET 했을 때 "권한 없음(403)" 으로 응답하면 ID 가 존재한다는 정보가 노출됩니다. 공격자가 ID 를 enumerate 해 *"누가 어떤 알림을 받았는지"* 를 추론할 수 있다고 봤습니다.
