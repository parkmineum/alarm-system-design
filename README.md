...
요구사항 자체가 아니라 *"왜 이걸 골랐는가"* 를 정리했습니다. 비교한 대안과 탈락 사유를 함께 남겨두면, 같은 자리에서 다시 고민하더라도 같은 결론에 도달할 수 있다고 생각했기 때문입니다.
...
요구사항 두 줄이 후보를 좁혔다고 생각했습니다.

- *"서버 재시작 후에도 미처리 알림이 유실 없이 재처리되어야 한다"*
- *"다중 인스턴스 환경에서 동일 알림이 중복 처리되지 않아야 한다"*

`@Async`, `ApplicationEvent`, 인메모리 큐는 모두 첫 번째 요구사항에 걸립니다 — 큐가 메모리에만 있으면 JVM 종료와 함께 사라지기 때문입니다. 두 번째 요구사항을 만족시키려면 분산 락이 추가로 필요해집니다.

DB 가 이미 영속성과 분산 락(`SELECT ... FOR UPDATE`)을 동시에 제공하므로, outbox 패턴이 **가장 적은 추가 인프라로 두 요구사항을 모두 만족시킨다고 판단했습니다**. 거기에 dispatcher 추상을 두면 미래에 Kafka 등으로 갈아탈 때 빈 교체 한 곳만 바꾸면 되는 — 점진적 마이그레이션 경로가 outbox 위에 자연스럽게 깔린다는 점도 함께 고려했습니다.
...
호출자가 키를 직접 보내는 모델도 후보로 두고 비교했습니다. 두 옵션이 짊어지는 위험의 성격이 서로 달랐습니다.
...
서버에서 결정적으로 생성하면 호출자는 *"같은 이벤트 = 같은 인자"* 만 보장하면 되고, 키 누락이라는 실패 모드 자체가 사라진다고 생각했습니다.

```
idempotencyKey = sha256(eventType + ':' + refType + ':' + refId + ':' + recipientId + ':' + channel + ':' + scheduledAt)
```

`scheduledAt` 까지 키에 포함시킨 이유는 같은 이벤트라도 다른 시각에 예약 발송하는 케이스 (D-1 알림 / D-3 알림) 는 별개 row 로 다뤄져야 한다고 판단했기 때문입니다.
...
ShedLock 같은 *스케줄러 단위* 락은 결국 동시성을 1로 떨어뜨리는 구조라고 판단했습니다 — 워커를 N 개 띄워도 한 번에 한 인스턴스만 일하게 됩니다. 처리량이 워커 수에 의존하지 않으면 수평 확장의 의미가 옅어집니다.

반면 `FOR UPDATE SKIP LOCKED` 는 **row 단위** 락이므로, 워커마다 락이 걸리지 않은 다음 row 를 가져가게 됩니다. 처리량이 워커 수만큼 선형으로 증가하고, 락 보유 시간을 짧게 (`PROCESSING` 마킹만 하고 즉시 commit, 실제 외부 호출은 락 밖) 유지하면 락 경합도 거의 발생하지 않는다고 판단했습니다.
...
MySQL 8.0.1+ 제약은 `docker-compose.yml` 의 `mysql:8.0` 이미지로 충족시켰습니다.
...
수동 재시도 시 `autoAttemptCount` 를 어떻게 다룰지가 가장 핵심적인 트레이드오프라고 생각했습니다.
...
분리 정책은 다음 두 가드레일과 함께 묶어서 적용했습니다.
...
세 시점을 후보로 두고 비교했습니다. 트레이드오프는 *"최신 템플릿 사용"* 과 *"실패 발견 시점"* 사이에 있다고 생각했습니다.
...
`rendered_body` 와 `template_version` 이 `notification` row 에 함께 저장되기 때문에, 추후 템플릿이 수정되어도 *"실제로 무엇을 보냈는지"* 를 추적할 수 있습니다.

`notification` 과 `notification_template` 을 FK 로 묶지 않은 이유도 같은 맥락에 있습니다. FK 로 묶어두고 템플릿을 수정하면 과거 발송 row 의 의미가 흔들리게 되므로, 스냅샷 분리가 더 안전하다고 판단했습니다.

---

### 6. 다중 디바이스 읽음 처리 — 조건부 UPDATE

같은 알림에 다른 디바이스가 동시에 `PATCH ../read` 를 보내면 `read_at` 이 마지막 요청 시각으로 덮어 씌워질 수 있습니다. SELECT 후 UPDATE 패턴은 락이 필요하고, 무조건 UPDATE 는 첫 읽음 시각을 잃게 됩니다. 조건부 UPDATE 한 줄로 세 가지를 동시에 해결할 수 있다고 판단했습니다.
...
`affected rows = 1` 이면 최초 읽음, `0` 이면 이미 읽음으로 처리되며 둘 다 `200 OK` 의 no-op 입니다. DB row write lock 이 자동 직렬화해 주므로 별도 락이 필요하지 않다고 판단했습니다.
...
요구사항이 *"실제 운영 환경 전환 가능한 구조"* 를 요구하고 있었기 때문에, 미래에 Kafka 등으로 갈아탈 수 있도록 swap 지점을 미리 노출시켜 두는 편이 낫다고 생각했습니다.
...
워커가 row 를 `PROCESSING` 으로 바꾼 직후 비정상 종료되면, 그 row 는 영원히 `PROCESSING` 상태로 남게 됩니다. 복구 잡(`@Scheduled`) 이 임계값을 넘긴 row 를 `PENDING` 으로 되돌리도록 두었습니다.

임계값을 결정할 때 고려한 트레이드오프는 다음과 같습니다.
...
복구된 row 는 `autoAttemptCount` 를 증가시키지 않도록 했습니다. 외부 호출 결과를 알 수 없는 상태이기 때문에, 카운터를 올려버리면 정상 동작 가능한 row 가 빠르게 DEAD_LETTER 로 떨어지는 부작용이 생긴다고 생각했습니다.
...
예약 발송을 별도 스케줄러 인프라(예: Quartz)로 둘지, 컬럼 한 개로 통합할지를 두고 고민했습니다.

`scheduled_at` 컬럼 하나로 통합하면 즉시 발송과 예약 발송이 **같은 코드 경로** 를 타게 됩니다.
...
별도 스케줄러를 두면 두 시스템(스케줄러 + outbox)의 정합성을 신경 써야 하므로, 컬럼 통합이 복잡도를 한 단계 낮춰준다고 판단했습니다.

---

### 10. 타인 알림 접근 — 404 vs 403

다른 사용자의 알림 ID 로 GET 했을 때 "권한 없음(403)" 으로 응답하면 ID 가 존재한다는 정보가 노출됩니다. 공격자가 ID 를 enumerate 해 *"누가 어떤 알림을 받았는지"* 를 추론할 수 있다고 생각했습니다.

그래서 `404 NOTIFICATION_NOT_FOUND` 로 통일했습니다. 이렇게 하면 존재 여부 자체가 외부에 드러나지 않게 됩니다.
...
같은 정책을 admin API 가 아닌 모든 사용자 API 에 일관되게 적용해 두었습니다.
